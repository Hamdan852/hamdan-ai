...
export default async function handler(req, res) {
  ...
  const type = clean(req.body?.type, 80) || "Custom";
  ...
  const files = [
    { path: "index.html", purpose: "Website entry page" },
    { path: "styles.css", purpose: "Responsive visual system" },
    { path: "app.js", purpose: "Interactive behavior" },
    { path: "README.md", purpose: "Project instructions and requirements" }
  ];

  return res.status(200).json({
    ok: true,
    stage: "Plan",
    project: { id: projectId, name, type, brief, repository, status: "Planned" },
    files,
    workflow: ["Understand", "Inspect", "Plan", "Approve", "Change", "Test", "Deploy", "Verify"],
    approvalRequired: true,
    writesPerformed: false,
    deploymentPerformed: false,
    message: "Website project plan created. Explicit approval is required before files are generated."
  });
}
